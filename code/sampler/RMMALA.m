...
w_curr = w(:,1);
s2_curr = eye(M)*2^2;%[];
[ll_curr,mu_curr,G_curr,Ginv_chol_curr,mu_base_curr] = computeRMMALAgaussian_CUDA(w_curr,X_gpu,Y_gpu,C_gpu,e,s2_curr,true);

for ss = 2:NS
    w_curr = w(:,ss-1);
    %s2_curr = eye(M)*0.1^2;%[];
    
    %[ll_curr,mu_curr,G_curr,Ginv_chol_curr,mu_base_curr] = computeRMMALA(w_curr,X,y,Xy,e);
    %[ll_curr,mu_curr,G_curr,Ginv_chol_curr,mu_base_curr] = computeRMMALAgaussian_CUDA(w_curr,X_gpu,Y_gpu,C_gpu,e,s2_curr);

    w_star = mu_curr + e*Ginv_chol_curr'*randn(size(mu_curr));
    %w_star = mvnrnd(mu_curr,e^2*inv(G_curr))';

    %[ll_star,mu_star,G_star,Ginv_chol_star,mu_base_star] = computeRMMALA(w_star,X,y,Xy,e);
    [ll_star,mu_star,G_star,Ginv_chol_star,mu_base_star] = computeRMMALAgaussian_CUDA(w_star,X_gpu,Y_gpu,C_gpu,e,s2_curr,true);
