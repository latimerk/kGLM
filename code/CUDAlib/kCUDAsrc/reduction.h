...
void getNumBlocksAndThreads(int whichKernel, int n, int maxBlocks, int maxThreads, int &blocks, int &threads) {
    ...
    {
        mexPrintf("n is too large, please choose a smaller number!\n");
    }

    if (blocks > prop.maxGridSize[0])
    {
        mexPrintf("Grid size <%d> excceeds the device capability <%d>, set block size as %d (original %d)\n",
...
template<class T>
void sumIntoBlocks(T * devPointer_i,T * devPointer_o, int &nb, const long int M, cudaStream_t stream) {
    int whichKernel = 6;
    int maxBlocks  = 1024;
    int maxThreads = 512;
    int numThreads = 512;
    ...
    reduce<T>((int)M, numThreads, nb, whichKernel, devPointer_i, devPointer_o, stream);
}
// Instantiate the reduction function for 3 types
template void
sumIntoBlocks<int>(int  * devPointer_i,int * devPointer_o, int &nb, const long int M, cudaStream_t stream);

template void
sumIntoBlocks<float>(float  * devPointer_i,float * devPointer_o, int &nb, const long int M, cudaStream_t stream);

template void
sumIntoBlocks<double>(double * devPointer_i,double * devPointer_o, int &nb, const long int M, cudaStream_t stream);
